...
import com.android.tempelate.databinding.ActivityMultiApiCallBinding
...
class MultipleAPICallActivity : AppCompatActivity() {

    private var dataBinding: ActivityMultiApiCallBinding? = null
    var viewModel: MultipleAPICallViewModel?= null
    var adapter: CricketerAdapter?= null

    override fun onCreate(savedInstanceState: Bundle?) {

        super.onCreate(savedInstanceState)
        dataBinding = DataBindingUtil.setContentView(this, R.layout.activity_multi_api_call)
        initializeViewModel()
        initViews()
        registerObserver()
    }
    ...
    private fun initializeViewModel() {
        viewModel = ViewModelProvider(this, ViewModelFactory(APICallHelperImp(RetrofitBuilder.apiService)))[MultipleAPICallViewModel::class.java]
    }
}